...
def resolve_backward_launch_config(forward_block_n: int, forward_num_warps: int, forward_num_stages: int) -> tuple[int, int, int]:
    """Return an opt-in backward-only Triton launch geometry for overlay experiments."""
    block_n = int(os.environ.get("DWARF_DSQG_BWD_BLOCK_N", forward_block_n))
    num_warps = int(os.environ.get("DWARF_DSQG_BWD_NUM_WARPS", forward_num_warps))
    num_stages = int(os.environ.get("DWARF_DSQG_BWD_NUM_STAGES", forward_num_stages))
    if block_n < 16 or block_n & (block_n - 1):
        raise ValueError(f"DWARF_DSQG_BWD_BLOCK_N must be a power of two >= 16, got {block_n}")
    if num_warps not in (1, 2, 4, 8):
        raise ValueError(f"DWARF_DSQG_BWD_NUM_WARPS must be one of 1,2,4,8, got {num_warps}")
    if num_stages < 1:
        raise ValueError(f"DWARF_DSQG_BWD_NUM_STAGES must be >= 1, got {num_stages}")
...
class _DSQGV18GroupedFn(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, pos_bias, scale_embed,
        ...
        BWD_BLOCK_N, BWD_NUM_WARPS, BWD_NUM_STAGES = resolve_backward_launch_config(BLOCK_N, _nw, _ns)
    ...
    def backward(ctx, dout):
        ...
        BN = ctx.BWD_BLOCK_N
        BHD = ctx.BLOCK_HD
        NW = ctx.BWD_NUM_WARPS
        NS = ctx.BWD_NUM_STAGES
        j_val = ctx.j_val
        j_small = ctx.j_small
        j_large = ctx.j_large
        J_PAD = ctx.J_PAD
