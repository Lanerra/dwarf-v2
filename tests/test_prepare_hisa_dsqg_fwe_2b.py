...
def test_build_dry_run_contract_targets_v2_hisa_hybrid_and_forbids_dsqg_w(tmp_path: Path) -> None:
    ...
    assert env["DWARF_LR_SCHEDULE"] == "wsd"
    assert env["DWARF_SCHEDULE_TOTAL_STEPS"] == "12500"
    assert env["DWARF_WSD_WARMUP_STEPS"] == "625"
    assert env["DWARF_WSD_STABLE_STEPS"] == "10000"
    assert env["DWARF_WSD_DECAY_STEPS"] == "1875"
    assert env["DWARF_SCHEDULE_STEP_OFFSET"] == "0"
