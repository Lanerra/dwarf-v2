"""
DWARF D=512 L=10 Triadic J=96 — HISA/DSQG hybrid v2 L3, hardened Dolma 3 Mix
20B raw/EOD base-pretraining trainer.

This isolated production copy preserves the general trainer while adding strict
mid-epoch continuation, atomic rolling checkpoints, and periodic validation.
Set DWARF_Q6_G128=1 only for the existing DSQG direct-gather smoke path.

Scratch target: train the Bible-Muon D512 architecture from random init on the
pinned Dolma 3 Mix 150B-derived, OLMo-1-tokenized 20B packed artifact.
...
NUM_LAYERS       = 10
DSR_LAYER        = 3
VOCAB_SIZE       = int(os.environ.get('DWARF_VOCAB_SIZE', '50282'))
...
def _restore_rng_state(state: dict) -> None:
    if not isinstance(state, dict):
        raise ValueError('checkpoint rng_state must be a dict')
...
def save_resumable_step_checkpoint(
    *,
    ...
    best_val_loss: float,
    ppl_results: dict,
    passkey_results: dict,
    checkpoint_config: dict,
) -> _pl.Path:
    if epoch < 1 or next_acc_step < 0 or global_step < 0:
        raise ValueError('checkpoint epoch, next_acc_step, and global_step must be non-negative with epoch >= 1')
    if epoch_indices.ndim != 1 or epoch_indices.dtype != torch.int64:
        raise ValueError('epoch_indices must be a one-dimensional int64 tensor')
    if not checkpoint_base_name or os.path.basename(checkpoint_base_name) != checkpoint_base_name:
        raise ValueError('checkpoint_base_name must be a filename stem')
    output_dir = _pl.Path(checkpoint_dir).resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    latest_path = output_dir / f'{checkpoint_base_name}_latest.pt'
    previous_path = output_dir / f'{checkpoint_base_name}_previous.pt'
    target_path = latest_path if checkpoint_path is None else _pl.Path(checkpoint_path).resolve()
    target_path.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = target_path.parent / f'.{target_path.name}.step{global_step}_{os.getpid()}.tmp'
    payload = {
        'checkpoint_kind': 'resumable_step_v1',
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'scheduler_state_dict': scheduler.state_dict(),
        'epoch': int(epoch),
        'next_acc_step': int(next_acc_step),
        'global_step': int(global_step),
        'epoch_indices': epoch_indices.detach().to(device='cpu', dtype=torch.int64).contiguous(),
        'best_val_loss': float(best_val_loss),
        'ppl_results': dict(ppl_results),
        'passkey_results': dict(passkey_results),
        'rng_state': _capture_rng_state(),
        'config': dict(checkpoint_config),
    }
    try:
        torch.save(payload, temporary_path, pickle_protocol=5)
        verified = torch.load(temporary_path, map_location='cpu', weights_only=False)
        if verified.get('checkpoint_kind') != 'resumable_step_v1' or verified.get('global_step') != global_step:
            raise ValueError('temporary checkpoint verification failed')
        if checkpoint_path is None and latest_path.exists():
...
def restore_resumable_step_checkpoint(
    *,
    ...
    required = {
        'checkpoint_kind', 'model_state_dict', 'optimizer_state_dict', 'scheduler_state_dict',
        'epoch', 'next_acc_step', 'global_step', 'epoch_indices', 'best_val_loss',
        'ppl_results', 'passkey_results', 'rng_state', 'config',
    }
    missing = sorted(required.difference(payload))
    if payload.get('checkpoint_kind') != 'resumable_step_v1' or missing:
        raise ValueError(f'not a complete resumable step checkpoint; missing={missing}')
    epoch_indices = payload['epoch_indices']
    if not torch.is_tensor(epoch_indices) or epoch_indices.ndim != 1 or epoch_indices.dtype != torch.int64:
        raise ValueError('resumable checkpoint epoch_indices must be a one-dimensional int64 tensor')
    ...
    return {
        ...
        'next_acc_step': int(payload['next_acc_step']),
        'global_step': int(payload['global_step']),
        'epoch_indices': epoch_indices.contiguous(),
        'best_val_loss': float(payload['best_val_loss']),
        'ppl_results': dict(payload['ppl_results']),
        'passkey_results': dict(payload['passkey_results']),
        'checkpoint_config': dict(payload['config']),
    }


def should_save_step_checkpoint(*, global_step: int, save_every_steps: int,
                                next_acc_step: int, steps_per_epoch: int) -> bool:
    return (
        save_every_steps > 0
        and global_step > 0
        and global_step % save_every_steps == 0
        and next_acc_step < steps_per_epoch
    )


def should_run_periodic_validation(*, global_step: int, eval_every_steps: int) -> bool:
    return eval_every_steps > 0 and global_step > 0 and global_step % eval_every_steps == 0


def parse_input_token_milestones(value: str) -> tuple[int, ...]:
    parsed = []
    for raw_token_count in value.split(','):
        token_count = raw_token_count.strip()
        if not token_count:
            continue
        count = int(token_count)
        if count <= 0:
            raise ValueError('input-token milestones must be positive integers')
        parsed.append(count)
    return tuple(sorted(set(parsed)))


def crossed_input_token_milestones(previous_input_positions: int, current_input_positions: int,
                                   milestones: tuple[int, ...]) -> tuple[int, ...]:
    if current_input_positions < previous_input_positions:
        raise ValueError('input positions cannot decrease')
    return tuple(
        milestone for milestone in milestones
        if previous_input_positions < milestone <= current_input_positions
    )
...
_DOLMA3_TOKENIZER_DEFAULT = str(
    _pl.Path(_project_root).parent / 'dataset-pipeline' / 'artifacts' / 'olmo1_base_chatml' / 'tokenizer.json'
)
TOKENIZER_CANDIDATES = [os.environ.get('DWARF_TOKENIZER', _DOLMA3_TOKENIZER_DEFAULT)]
...
CHECKPOINT_DIR    = os.environ.get('DWARF_CHECKPOINT_DIR', 'autoresearch/checkpoints/dolma3_20b_hardened')
_CKPT_BASE_DEFAULT = 'd512_l10_dolma3_20b_hardened'
...
SAVE_EVERY_STEPS = int(os.environ.get('DWARF_SAVE_EVERY_STEPS', '500'))
EVAL_EVERY_STEPS = int(os.environ.get('DWARF_EVAL_EVERY_STEPS', '1000'))
MILESTONE_INPUT_TOKENS = parse_input_token_milestones(
    os.environ.get(
        'DWARF_MILESTONE_INPUT_TOKENS',
        '500000000,1000000000,2000000000,5000000000,10000000000,15000000000,20000000000',
    )
)
if SAVE_EVERY_STEPS < 0 or EVAL_EVERY_STEPS < 0:
    raise ValueError('DWARF_SAVE_EVERY_STEPS and DWARF_EVAL_EVERY_STEPS must be non-negative')
DATASET_PATH = os.environ.get(
    'DWARF_DATASET',
    'datasets/dwarf_dolma3_mix150b_olmo1chatml_2048_20b/dataset.pt',
)
...
class DSQGAttentionV19Q6G128Smoke(DSQGAttentionV19):
    """q6_g128 Triton direct-gather K/V smoke path retained by the hardened trainer.
...
def train():
    ...
    print('=' * 70)
    if PURE_DSQG_BASELINE:
        print('  DWARF D512-L10 Dolma3-20B Hardened — PURE DSQG-D v1 control + R_PLANES=4, TIED LM_HEAD')
        print(f'  DSR/HISA: disabled; L{DSR_LAYER} uses DSQG-A control slot')
    else:
        print(f'  DWARF D512-L10 Dolma3-20B Hardened — {HISA_IMPL.upper()} HISA@L{DSR_LAYER} + R_PLANES=4, TIED LM_HEAD')
        print(f'  DSR@L{DSR_LAYER}: {HISA_IMPL_LABEL}(C={NUM_CHUNKS}, top_k={TOP_K_CHUNKS}, HISA_m={HISA_TOP_M_TOKENS})')
    print('  SCRATCH PRETRAINING: pinned Dolma 3 Mix 150B-derived 20B raw/EOD packed artifact.')
    print('=' * 70)
    if torch.cuda.is_available():
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        if resume_probe.get('checkpoint_kind') == 'resumable_step_v1':
            if skip_sched or os.getenv('DWARF_SKIP_OPT', '0') == '1' or reset_paged_adam:
                raise ValueError('resumable step checkpoints require strict optimizer and scheduler restoration')
            step_resume_state = restore_resumable_step_checkpoint(
                checkpoint_path=resume_path,
                model=model,
                optimizer=optimizer,
                scheduler=scheduler,
            )
            start_epoch = step_resume_state['epoch']
            resume_optimizer_state = {'mode': 'strict_step_restore', 'loaded': ['optimizer', 'scheduler'], 'skipped': []}
            print(
                f"  [resume] strict step checkpoint global_step={step_resume_state['global_step']} "
                f"epoch={step_resume_state['epoch']} next_acc_step={step_resume_state['next_acc_step']}",
                flush=True,
            )
            resume_path = ''
    if resume_path and os.path.isfile(resume_path):
    ...
    best_val_loss = float('inf') if step_resume_state is None else step_resume_state['best_val_loss']
    passkey_results = {} if step_resume_state is None else step_resume_state['passkey_results']
    ppl_results = {} if step_resume_state is None else step_resume_state['ppl_results']
    global_step = 0 if step_resume_state is None else step_resume_state['global_step']
    ...
    checkpoint_config['training']['save_every_steps'] = SAVE_EVERY_STEPS
    checkpoint_config['training']['eval_every_steps'] = EVAL_EVERY_STEPS
    checkpoint_config['training']['milestone_input_tokens'] = list(MILESTONE_INPUT_TOKENS)
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        if step_resume_state is not None and epoch == step_resume_state['epoch']:
            indices = step_resume_state['epoch_indices']
            if len(indices) != len(train_data):
                raise ValueError(
                    f'resumed epoch permutation rows={len(indices)} do not match train rows={len(train_data)}'
                )
            first_acc_step = step_resume_state['next_acc_step']
            if first_acc_step >= steps_per_epoch:
                raise ValueError('resumable step checkpoint cursor is already at or beyond the epoch boundary')
        else:
        ...
        for acc_step in range(first_acc_step, steps_per_epoch):
            t0 = time.time()
            ...
            if should_run_periodic_validation(global_step=global_step, eval_every_steps=EVAL_EVERY_STEPS):
                periodic_val_loss = evaluate(model, val_data, device, val_loss_mask)
                periodic_val_ppl = math.exp(min(periodic_val_loss, 20))
                ppl_results[global_step] = periodic_val_ppl
                if periodic_val_loss < best_val_loss:
                    best_val_loss = periodic_val_loss
                    torch.save(
                        {
                ...
                print(
                    f'  [periodic-eval] global_step={global_step} val_ppl={periodic_val_ppl:.2f} '
            ...
            if should_save_step_checkpoint(
                ...
                next_acc_step=acc_step + 1,
                steps_per_epoch=steps_per_epoch,
            ):
                checkpoint_path = save_resumable_step_checkpoint(
                    checkpoint_dir=CHECKPOINT_DIR,
                    checkpoint_base_name=CKPT_BASE_NAME,
                    model=model_ref,
                    optimizer=optimizer,
                    scheduler=scheduler,
                    epoch=epoch,
                ...
                print(
                    f'  [checkpoint] global_step={global_step} next_acc_step={acc_step + 1} path={checkpoint_path}',
                    flush=True,
                )

            input_positions_per_step = BATCH_SIZE * GRAD_ACCUM * MAX_SEQ_LEN
            previous_input_positions = (global_step - 1) * input_positions_per_step
            current_input_positions = global_step * input_positions_per_step
            for milestone_input_tokens in crossed_input_token_milestones(
                previous_input_positions,
                current_input_positions,
                MILESTONE_INPUT_TOKENS,
            ):
                milestone_path = save_resumable_step_checkpoint(
                    checkpoint_dir=CHECKPOINT_DIR,
                    checkpoint_base_name=CKPT_BASE_NAME,
                    checkpoint_path=os.path.join(
                        CHECKPOINT_DIR,
                        f'{CKPT_BASE_NAME}_tokens{milestone_input_tokens}.pt',
                    ),
                    model=model_ref,
                    optimizer=optimizer,
                    scheduler=scheduler,
                ...
                print(
                    f'  [milestone] input_tokens={milestone_input_tokens} global_step={global_step} path={milestone_path}',
                    flush=True,
                )
        ...
        ppl_results[global_step] = val_ppl
