...
RUN_ROOT = Path(os.environ.get("RUN_ROOT", ROOT / "runs" / f"dsqg_w_prior_noquota_200k_{STAMP}"))
VARIANT = os.environ.get("VARIANT", "w_prior_noquota_200k_scratch")
...
def train() -> dict[str, Any]:
    ...
    rc = run_cmd(cmd, env={"PYTHONUNBUFFERED": "1"}, label=f"train {VARIANT}")
    metrics = parse_train_log(out_dir / "trainer.stdout.log")
    metrics.update({"returncode": rc, "variant": VARIANT, "train_seqs": TRAIN_SEQS, "out_dir": str(out_dir)})
...
def main() -> int:
    ...
    write_json(RUN_ROOT / "screen_config.json", {
        "run_root": RUN_ROOT,
        "variant": VARIANT,
        "mode": "scratch_200k_fixed_subset",
        "train_gpu": TRAIN_GPU,
        "eval_gpu": EVAL_GPU,
    ...
    if t.get("returncode") != 0:
        write_json(RUN_ROOT / "metrics" / "screen_verification.json", {"pass": False, "errors": ["train failed"], "train": t})
        return 2
    e = run_evidence()
    s = run_semantic()
    x = run_external_trio()
    v = verify(t, e, s, x)
    write_json(RUN_ROOT / "metrics" / "stage_200k_final.json", {"train": t, "evidence": e, "semantic": s, "external_trio": x, "verification": v})
    write_json(RUN_ROOT / "metrics" / "screen_verification.json", v)
    log(f"screen verification: {json.dumps(v, sort_keys=True)}")
    log("200K screen complete")
